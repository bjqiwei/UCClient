...
lwsclose:
	...
	bool WebSocketServer::InitInstance()
	{
		...
		CDuiString strCertPath = CPaintManagerUI::GetInstancePath();
		strCertPath.Append(m_cert_path.c_str());
		m_Info.ssl_cert_filepath = strCertPath;//m_cert_path.c_str();
		CDuiString strKeyPath = CPaintManagerUI::GetInstancePath();
		strKeyPath.Append(m_key_path.c_str());
		//m_Info.ssl_ca_filepath = m_cert_path.c_str();
		m_Info.ssl_private_key_filepath = strKeyPath;// m_key_path.c_str();

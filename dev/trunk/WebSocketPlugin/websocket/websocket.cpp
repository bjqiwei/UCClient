...
lwsclose:
			...
			if ( wsclient && wsclient->m_state != CLOSING) {
				...
				if (wsclient->m_sendBuf.size() > 0) {
					...
					if (bufLen > 0)
					{
						...
						if (reader.parse(sdata, root) && root["cmd"].isString() && root["cmd"].asString() == "ping") {
						}
						else
	...
	bool WebSocketServer::InitInstance()
	{
		...
		m_Info.options = m_Info.options | LWS_SERVER_OPTION_DO_SSL_GLOBAL_INIT | LWS_SERVER_OPTION_ALLOW_NON_SSL_ON_SSL_PORT | LWS_SERVER_OPTION_REQUIRE_VALID_OPENSSL_CLIENT_CERT;
		m_Info.options = m_Info.options | LWS_SERVER_OPTION_VALIDATE_UTF8;

		std::string cert_path = utf8Dir  + m_cert_path;
		std::string key_path = utf8Dir + m_key_path;

		m_Info.ssl_cert_filepath = cert_path.c_str();
		//m_Info.ssl_ca_filepath = cert_path.c_str();
		m_Info.ssl_private_key_filepath = key_path.c_str();

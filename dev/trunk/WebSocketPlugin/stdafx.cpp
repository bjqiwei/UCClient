...
// WebCall.pch will be the pre-compiled header
...
void SetRegKey(CString name, CString strValue)
{
	...
	LONG lResult = shKey.Create(HKEY_CURRENT_USER, "SoftWare\\ytx\\WebCall");
	log4cplus::Logger log = log4cplus::Logger::getInstance("WebCall");
	if (lResult != ERROR_SUCCESS)
	{
		LOG4CPLUS_ERROR(log, _T("Create WebCall RegKey failed."));
	}
	else {
...
CString ReadRegKeyString(CString name, CString default)
{
	...
	lResult	= shKey.Open(HKEY_CURRENT_USER, "SoftWare\\ytx\\WebCall", KEY_READ);
	...
	if (lResult != ERROR_SUCCESS){
		lResult = shKey.Open(HKEY_USERS, ".DEFAULT\\SoftWare\\ytx\\WebCall", KEY_READ);
		if (lResult == ERROR_SUCCESS) {
			dwLen = MAX_PATH;
			lResult = shKey.QueryStringValue(name, strValue.GetBuffer(dwLen), &dwLen);
		}
	...
	if(lResult != ERROR_SUCCESS) {
		LOG4CPLUS_ERROR(log, _T("Open WebCall RegKey failed."));
	}
	else {
		LOG4CPLUS_DEBUG(log, __FUNCTION__" " << name << ":" << strValue);
	}
